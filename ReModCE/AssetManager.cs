...
namespace NEKOClient
{
    internal static class AssetManager
    {
        ...
        private static GameObject LoadPrefab(string @object)
        {
            if (_bundle is null)
            {
                NEKOClient.Error($"[0011]Failed to load Prefab: {@object}");
            ...
            if (go != null)
            {
                ...
                NEKOClient.Debug($"Loaded Prefab: {@object}");
                return go;
            }

            NEKOClient.Error("[0012]Failed to load Prefab: " + @object);
        ...
        private static Sprite LoadSprite(string sprite)
        {
            if (_bundle is null)
            {
                NEKOClient.Error($"[0013] Failed to load Sprite: {sprite}");
                throw new FileLoadException();
            }
            ...
            if (sprite2 != null)
            {
                ...
                NEKOClient.Debug($"Loaded Sprite: {sprite}");
                return sprite2;
            }

            NEKOClient.Error("[0014] Failed to load Sprite: " + sprite);
        ...
        {
            using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("NEKOClient.Resources.NEKOClient");
            if (stream != null)
            {
                ...
                {
                    NEKOClient.Error($"[0015] Nameplate Assets failed to load\n\n{e}");
                }
            }
            else
            {
                NEKOClient.Error("[0016] Stream is null, Nameplates cannot load");
            }
